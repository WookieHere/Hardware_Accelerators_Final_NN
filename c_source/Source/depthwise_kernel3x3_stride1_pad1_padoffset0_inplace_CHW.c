...
void depthwise_kernel3x3_stride1_pad1_padoffset0_inplace_kernel_CHW(
...
{
    printf("depthwise_kernel3x3_stride1_pad1_padoffset0_inplace_kernel_CHW start\n");
    ...
    for (c = 0; c < input_ch; c++){
        cols_8b = (q7_t*)(cols_8b_start + 1 * ((input_x) + 1 + 1)); //skip 1 rows
        src = input;
        for(i = 0; i < input_y; i++){
        ...
        depthwise_kernel3x3_stride1_pad1_padoffset0_inplace_kernel_CHW(output_y, output_x, bias++, biasR++, ksrc, output_mult++, output_shift++, inplace_out,output_offset,output_activation_min, output_activation_max,cols_8b_start, input_x, input_ch);
        ksrc += 9;
        input++;
    }
