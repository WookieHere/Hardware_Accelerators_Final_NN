                ...
                const uint16_t output_ch, q15_t *runtime_buf, q7_t pad_value)
{
    ...
    for (c = 0; c < input_ch; c++){
        cols_8b = (q7_t*)(cols_8b_start + 0 * ((input_x) + 0 + 1)); //skip 0 rows
        src = input;
        for(i = 0; i < input_y; i++){
        ...
        depthwise_kernel3x3_stride2_pad0_padoffset1_inplace_kernel_CHW(output_y, output_x, bias++, biasR++, ksrc, output_mult++, output_shift++, inplace_out,output_offset,output_activation_min, output_activation_max,cols_8b_start, input_x, input_ch);
        ksrc += 9;
        input++;
    }
        ...
        q7_t *cols_8b_iterptr, const uint16_t column_x, int channel_offset)
{
    ...
    for (i = 0; i < output_y; i++) {
        ...
        cols_8b_iterptr += 0 + 1 - (column_x & 1);
        cols_8b_iterptr += (STRIDE - 1) * (column_x + 0 + 1);
    }
}
